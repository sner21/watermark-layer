<template>
	<a-row style="width: auto;height: 100vh;flex-flow: nowrap;overflow: hidden">
			...
			<a-space direction="vertical" align="center" style="width: 100%;" :size="20">
				<div>
					<div style="display: flex">
						...
						<a-button @click="$refs.tela.appendRepeatText($refs.textRepeatInput.$el.value,index++)">
							添加平铺文字
						</a-button>
					</div>
					<div style="display: flex">
						<a-input type="text" ref="textInput"></a-input>
						<a-button @click="$refs.tela.appendText($refs.textInput.$el.value,index++)">添加文字</a-button>
					</div>
				...
				<div style="display: flex">
					...
					<a-button @click="$refs.tela.appendStamp(stampUrl, index++)">添加图片</a-button>
				</div>
				<div>
					<a-select :value="fontFamily[d]" @change="$refs.tela.editText(()=>fontFamily[d] = $event)">
					...
					<a-select :value="fontStyle[d]" @change="$refs.tela.editText(()=>fontStyle[d] = $event)">
						<a-select-option value="normal">正常</a-select-option>
						<a-select-option value="italic">斜体</a-select-option>
						<a-select-option value="oblique">倾斜</a-select-option>
					</a-select>
					<a-select :value="fontSize[d]" @change="$refs.tela.editText(()=>fontSize[d] = $event)">
				...
				<editer-option v-model:value="stampOpicity[d]" :min="0" :max="1" step="0.1"
				               @update:value="$refs.tela.editText()"
				...
				<editer-option v-model:value="stampMarginTop[d]" @update:value="$refs.tela.editText()" :min="0"
				               :max="props.size?.width"
				               :precision="0" label="纵向间距" :disabled="stampClass[d]!=='repeat'">
				</editer-option>
				<editer-option v-model:value="stampMarginLeft[d]" @update:value="$refs.tela.editText()" :min="0"
				               :max="props.size?.width"
				...
				<editer-option v-model:value="stampWidth[d]" :min="0" :max="props.size?.width-1"
				               :precision="0" label="宽度" :disabled="!stamp[d]">
				</editer-option>
		...
		<a-col :span="12"
			...
			<canvas-main ref="tela" :stampList="stampList" :size="props.size" :canvas="props.canvas"
